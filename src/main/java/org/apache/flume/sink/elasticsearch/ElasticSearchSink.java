...
import static org.apache.flume.sink.elasticsearch.ElasticSearchSinkConstants.THREAD_NUM;
import static org.apache.flume.sink.elasticsearch.ElasticSearchSinkConstants.TIME_ZONE;
import static org.apache.flume.sink.elasticsearch.ElasticSearchSinkConstants.INDEX_NAME_DATE_FORMAT;
...
public class ElasticSearchSink extends AbstractSink implements Configurable {
    ...
    public Status process() throws EventDeliveryException {
        ...
        try {
            ...
            for (count = 0; count < batchSize; ++count) {
                ...
                if (event == null) {
                    logger.info("get empty event from channel : "+indexName);
                    break;
                }
                String realIndexType = BucketPath.escapeString(indexType, event.getHeaders());
                client.addEvent(event, indexNameBuilder, realIndexType, ttlMs, count);
            }
            if (count <= 0) {
    ...
    public void configure(Context context) {
        ...
        if (StringUtils.isNotBlank(context.getString(THREAD_NUM))) {
            this.threadNum = Integer.parseInt(context.getString(THREAD_NUM));
        }
        ...
        if (StringUtils.isNotBlank(context.getString(TIME_ZONE))) {
            this.timeZone = context.getString(TIME_ZONE);
        }
        if (StringUtils.isNotBlank(context.getString(INDEX_NAME_DATE_FORMAT))) {
            this.indexNameDateFormat = context.getString(INDEX_NAME_DATE_FORMAT);
        }
        ...
        elasticSearchClientContext.put(INDEX_NAME,indexName); //add by liz 11.27
        ...
        try {
            ...
            //ADD BY liz 11.27
            indexnameBuilderContext.put(TIME_ZONE, timeZone);
            indexnameBuilderContext.put(INDEX_NAME_DATE_FORMAT, indexNameDateFormat);
        ...
        try {
            if (isLocal) {
                ...
                client = clientFactory.getClient(clientType, serverAddresses,
                        clusterName, eventSerializer, indexRequestFactory, threadNum);
